...
static void vplic_set_enbl(struct vcpu* vcpu, int vcntxt, irqid_t id, bool set)
{
    ...
    if (id <= PLIC_MAX_INTERRUPTS && vplic_get_enbl(vcpu, vcntxt, id) != set) {
        ...
        if(vplic_get_hw(vcpu, id)){
            int pcntxt_id = vplic_vcntxt_to_pcntxt(vcpu, vcntxt);
            /* SONDE 2026-09-12 : voie MATERIELLE, le PLIC physique est
             * programme. C'est ce qu'on doit voir pour les irq 12 et 13. */
            INFO("ENBL hw irq %d set=%d pcntxt=%d", id, set, pcntxt_id);
            plic_set_enbl(pcntxt_id, id, set);
        } else {
            /* SONDE : voie VIRTUELLE. Si les irq 12 et 13 passent par la, Bao
             * les traite comme purement virtuelles et le PLIC physique n'est
             * jamais arme -- ce serait la cause du silence. */
            INFO("ENBL virt irq %d set=%d (PLIC physique NON arme)", id, set);
            vplic_update_hart_line(vcpu, vcntxt);
        }
    }
    spin_unlock(&vplic->lock);
